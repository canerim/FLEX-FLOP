...
residual blocks with $j=2$, on $256\times256$ tiles, evaluated on the $40$ CTC
sequences available without JVET credentials (UVG, MCL-JCV, HEVC class~E;
classes B, C and D are not measured). Decibels here pool every tile of every
frame into a single MSE, which is the form matching the Lagrangian
$J=D+\lambda C$ of Definition~\ref{def:setup}; the codec convention of averaging
a per-frame decibel reads $0.023$--$0.033$\,dB higher on an identical
assignment, so figures quoted against published DCVC-UF results use that one
instead. Distortion is measured against the \emph{released} decoder from the
identical bitstream, so the encoder, hyperprior and entropy model are
...
leaving early grows $3.3\times$ from the lowest to the highest rate.}
\label{tab:exits}
\begin{tabular}{lrrrrr}
\toprule
QP & exit 2 & exit 3 & exit 4 & exit 5 & bpp \\
\midrule
0  & 0.1672 & 0.0547 & 0.0321 & 0.0028 & 0.2226 \\
32 & 0.3245 & 0.0985 & 0.0547 & 0.0142 & 0.2966 \\
63 & 0.5592 & 0.1443 & 0.0713 & 0.0286 & 0.5466 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
0  & 0.30\% & 0.39\% & 0.45\% & 0.03\% \\
16 & 0.65\% & 0.74\% & 0.38\% & 0.01\% \\
32 & 0.62\% & 1.41\% & 0.37\% & 0.00\% \\
48 & 0.62\% & 2.00\% & 0.30\% & 0.00\% \\
63 & 0.68\% & \textbf{2.09\%} & 0.27\% & 0.00\% \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
0  & 0.0048 & 0.0080 & 0.0113 & 0.0168 \\
32 & 0.0074 & 0.0117 & 0.0177 & 0.0273 \\
63 & 0.0110 & 0.0181 & 0.0284 & 0.0461 \\
\bottomrule
\end{tabular}
\end{table}
...
The magnitudes here are smaller than on the weaker checkpoint this table was
first computed from, where $\Delta/J$ ran $0.91\%$ to $6.86\%$ instead of
$0.39\%$ to $2.09\%$, and that direction is not accidental. By
Theorem~\ref{thm:gain}, $\Delta$ vanishes exactly when every tile prefers the
same exit. Improving the shallow exits (exit~2's penalty at $\mathrm{qp}\,0$
fell from $0.3093$ to $0.1672$\,dB) compresses the differences between exits,
so more tiles agree on the argmin and the measured value of adaptivity falls.
Adaptivity is worth less on a better ladder, while the ladder saves more: the
same checkpoint moves the compute saved at $0.1$\,dB from $28.3\%$ to $34.7\%$
at $\mathrm{qp}\,0$. $\Delta$ prices routing against a fixed ladder and must be
read against the ladder it was measured on.


\begin{table}[h]\centering\small
\caption{Test of condition~\eqref{eq:logconvex} along the measured per-tile
frontier. Convexity of a sampled curve is tested
directly, as non-decreasing secant slopes of $\ln D$ against $S$; the margin is
the smallest increase attained. The condition holds on every point of every
frontier, and the margin grows tenfold from the lowest rate to the highest, so
$\mathrm{dB}(S)$ is convex here, by Proposition~\ref{prop:logconvex} and by
measurement, rather than by assumption. Fitting a quartic and differentiating
it twice instead reports $69$--$78\%$, but those violations sit entirely at the
$42.5\%$ ceiling, where the frontier is near-vertical and a quartic overshoots;
the discrete test has no such failure mode and no fitted degree to choose.}
\label{tab:logconvex}
\begin{tabular}{lrrr}
\toprule
QP & frontier points & condition holds & margin \\
\midrule
0  & 16 & 100.0\% & 0.0024 \\
16 & 14 & 100.0\% & 0.0050 \\
32 & 13 & 100.0\% & 0.0088 \\
...
\begin{remark}[The frontier has a floor, and it is not free]
Every quantity here is measured against a fixed reference decoder, and the
deepest exit is assumed to reproduce it. Nothing in the derivation enforces
that: the exits are trained, and if the deepest one drifts by $\delta$ then the
whole achievable set shifts up by $\delta$ before any compute is saved. A
budget below $\delta$ is then unreachable at every assignment. Routing has not
failed; $\mathcal{A}$ simply does not intersect the constraint.

This is not hypothetical. Training the same ladder under the released recipe
with no term tying the deepest exit to the reference leaves
$\delta = 0.146$--$0.217$~dB after one epoch, against $0.003$--$0.033$~dB with
one, over $\mathrm{qp}\,0$ to $63$ on the same $40$ sequences and the same
$256\times256$ tiles. The first exceeds a $0.1$~dB budget outright: its frontier
spans $[0.223, 0.231]$~dB and does not overlap the second's range at all, so the
two cannot even be compared by an integral over a shared interval.

The floor therefore belongs to the setup rather than to the method. Reporting a
saving without it states where a curve sits on an axis whose origin has moved.
\end{remark}
