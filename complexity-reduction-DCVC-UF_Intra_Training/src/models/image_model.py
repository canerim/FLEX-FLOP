...
class DMCI(CompressionModel):
    ...
    def forward_one_frame(self, x, qp, recon_only=False):
        ...
        return {
            'x_hat': x_hat,
            # Exposed for knowledge distillation (train_image_distill.py); the
            # quantised latent is the natural target because matching it makes
            # the student inherit the teacher's compressibility. Unused keys are
            # ignored by loss_func / get_loss_info.
            'y_hat': y_hat,
            'mse': mse,
            'bpp': bpp,
            'bits_y': bits_y,
    ...
    def forward(self, x, qp, lambdas=None, get_loss_info=False, recon_only=False):
        ...
        if get_loss_info:
            _, _, H, W = x.size()
            pixel_num = H * W
            info = self.get_loss_info(result, loss, pixel_num, x=x)
