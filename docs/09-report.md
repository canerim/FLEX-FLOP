...
**A — the encoder searches and signals the map.** It holds the source, so it decodes all K exits, measures the true error, and takes the optimum `k*(t) = argmin_k [MSE(t,k) + λ·c_k]`, with one λ bisected per rate against a real decode of the resulting map. Not an estimate — the oracle. The exact search costs the *encoder* 4.5 decodes per frame (1.4 for the full-frame approximation, which agrees on 85% of tiles) and adds 79–95 bits/frame depending on rate, 0.008–0.020% of the bitrate. Needs one new bitstream field, so both ends must agree. This is also the conventional choice: HEVC and VVC signal partitioning and mode rather than having the decoder guess.
...
| qp | rate-rank | B router | A oracle |
|---|---|---|---|
| 0 | 41.91 | 41.75 | 41.91 |
...
**Every wall-clock number was timed on the wrong device.** `torch.cuda.Event` is created on the process's *current* device, not on the device the tensors live on, and the timing runs used a non-default GPU. It does not raise — it returns numbers. The tell was a released decoder that appeared to take 274, 401 and 512 ms at qp 0, 32 and 63: a fixed synthesis network whose arithmetic does not depend on the quality index cannot slow down with it. It takes 111 ms at all three. The tiling overhead falls from 8.5% to 3.6%, and the claim that the unsorted per-tile loop was *slower* than the dense decoder — a headline finding — does not survive: it realises 27.9% at qp 0 against a 35.3% prediction, and sorting adds 1.2 points rather than rescuing it. `tests/test_timing_device.py` now asserts that every script using CUDA events pins the device, and it found two more. [DECISIONS 88](../DECISIONS.md).

**The router's exit mask stopped being a mask.** The head suppresses exits below the split depth by assigning −1e4, which works only while its own logits stay well above that. They had drifted to it — nothing in a cross-entropy or a regret objective penalises a common offset — so the two suppressed entries were the *largest* in every row, took nearly all the probability mass, and the caller's clamp turned the result into the cheapest real exit. A large share of every configuration-B allocation went to the cheapest rung for a reason unrelated to the tile. Fixed to −inf, with a test that checks the property across six logit scales instead of the constant. The A−B gap goes from 1.7→13.3 points to a roughly flat 3.1–5.2, and the rate-rank crossover reverses. [DECISIONS 89](../DECISIONS.md).
