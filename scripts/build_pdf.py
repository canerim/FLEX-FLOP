...
def content(colw, fullw):
    ...
    par(r"<b>Where the β above came from.</b> Every β in the signalled-against-"
        ...
        r"β came from.")
    par(r"The held-out β misses the budget, and mostly on the high side: it "
        r"delivers between \HeldDbBest and \HeldDbWorst dB where 0.1 dB was "
        r"asked for, overshooting at \HeldNOver of the \HeldNHeld rates it "
        r"covers by \HeldOverMin–\HeldOverMax dB, which is "
        r"\HeldOverPctMin–\HeldOverPctMax% of the budget, and undershooting "
        r"at q\HeldUnderMaxQp by \HeldUnderMax. Distortion and saving move "
        r"together, so those rows also report more saving, up to "
        r"\HeldGiveUpMinAbs points more at q\HeldGiveUpMinQp, and reading "
        r"the column straight against [[tab:ab]] would credit the router with "
        r"compute it bought using quality the budget did not allow. The last "
        r"column takes that back out: at the quality the held-out β actually "
        r"delivered, the two allocations agree to within "
        r"\HeldTransferAbsMax points. What moves between the two sets is the "
