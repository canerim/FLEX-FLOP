...
# Figure numbers, resolved before anything is laid out.
#
# The prose could not name a figure. _autonum numbers captions as they are
# emitted, so a sentence written before the figure has no number to use, and
# twenty-six of thirty-five figures were left with no sentence pointing at them
# -- a figure a reader is never sent to. This reads the emission order out of
# this file's own source (the banner, then content()'s figures in order; there
# are no loops, so source order is emission order) and lets prose write
# [[fig:name]], which sub() turns into "Figure N".
def _figure_numbers():
    src = Path(__file__).read_text()
    # The banner is emitted into the story before content() runs, but it is
    # written after content() in this file, so source order is emission order
    # for everything except that one. It is always Figure 1.
...
def _table_numbers():
    """Same idea for tables. tbl() calls in source order are emission order."""
    src = Path(__file__).read_text()
    out = {}
...
def sub(t):
    ...
    def _tabref(m):
        k = m.group(1)
        if k not in TBLN:
            raise KeyError(f"[[tab:{k}]] names no table in this document")
        return f"Table {TBLN[k]}"
    t = re.sub(r"\[\[fig:([A-Za-z_0-9]+)\]\]", _figref, t)
    t = re.sub(r"\[\[tab:([A-Za-z_0-9]+)\]\]", _tabref, t)
...
def content(colw, fullw):
    ...
    par(r"Learned codecs are compared on rate and distortion, with complexity "
        ...
        r"milliseconds ([[fig:field]]). That number is a constant, and it is constant for a "
    ...
    par(r"Three lines of work meet in this paper and none of them quite "
        r"contains it ([[tab:positioning]]). Early exit made depth a per-input decision and left it "
    ...
    par(r"The construction is short. A frame is cut into tiles once the "
        r"shared part of the decoder has run ([[fig:patchify]]), each tile is allowed to leave "
    ...
    par(r"<b>A</b> decides at the encoder. The encoder holds the source, so it "
        ...
        r"tile ([[fig:allocation]]), which is the analogue of the mode map a standard codec "
        r"signals. We report its cost as the entropy of the map's own "
        r"per-frame histogram, \MapBits bits for a 1080p frame, which is "
        r"\MapOverheadLow% of a typical bitrate; that is what an arithmetic "
        r"coder would spend on it rather than the two-bits-per-tile worst "
        r"case, and it is an upper bound in one respect we have left in place. "
        r"Below the split depth the exit indices are not distinct, so a real "
        r"codec would signal four symbols where this entropy counts six. "
        r"Measured, that inflates the figure by at most seven bits a frame. "
        r"The map costs the encoder about one extra decode per frame and the "
        r"decoder nothing.")
    ...
    par(r"Two consequences follow from the ladder, and both are easy to state "
        ...
        r"therefore has K−j distinct costs ([[fig:ladder]]). The second consequence is the "
    ...
    h2("3.5 Allocation")
    par(r"The ladder and its costs are fixed by this point ([[fig:mechanism]]), and one decision is "
    ...
    par(r"<b>The head, exactly.</b> Three things enter it, and the third is "
        r"the one that earns its parameters; [[fig:router_arch]] gives every "
        r"shape and count. The first is the feature at the split point, the "
        r"last thing every tile shares. The second is the decoded latent ŷ "
        r"with the entropy model's scales σ [2], the predicted Gaussian "
        r"width used to code each latent position: σ is already computed "
        r"during the decode and is, position by position, an estimate of how "
        r"hard that position was to code. The third is the quality index. "
        r"Each of the first two is projected by a 1×1 and reduced to one "
        ...
        r"stem, which is the only part that runs per pixel.")
    ...
    par(r"If padding is the estimator, the obvious next question is what a "
        ...
        r"depend on it at all, and the damage scales with the tile perimeter ([[tab:tilesize]]), "
    ...
    par(r"The largest factor of all is the easiest one to miss. On the "
        ...
        r"weights learning to live with it ([[fig:seam_vs_qp]]). That single change removes more of "
    ...
    par(r"Only 0.29% of each block has spatial extent, so the exact fix is "
        ...
        r"stitched frame and has no full-frame counterpart. Measured over "
        r"\HaloNFrames frames and three rates: \HaloFilterOn with the filter "
        r"on, <b>exactly \HaloFilterOff</b> with it off, against \HaloPadding "
    ...
    par(r"Panel c shows the distribution that the headline averages over, "
    ...
    par(r"The fall in saving with rate is the allocation walking down the "
        r"ladder ([[fig:exit_vs_rate]]). At the lowest rate \ExitShallowLow% of tiles take the "
    ...
    par(r"One multiplier is bisected for the whole test set, so the budget "
        ...
        r"they were nominally given ([[tab:setbudget]]), and the worst single sequence is "
    ...
    par(r"The third of those has a practical edge. The sweep reaches only hull "
        ...
        r"is at most 0.05 saving points ([[fig:theory]]). For this problem the standard "
    ...
    par(r"The collapse appears robust across two checkpoints. We "
        ...
        r"curve collapses the same way, though the power law fits it less well: "
        r"R² = \BandRTwoB against \BandRTwo here, and \BandFitErrB points of "
        r"worst-case error against \BandFitErr. What replicates is the "
        r"collapse, not the quality of the one-parameter description of it. "
        r"The <i>exponent</i> "
        r"does not carry over either: \BandExpB there against \BandExp here, so it "
    ...
    par(r"<b>Where the β above came from.</b> Every β in the signalled-against-"
        ...
        r"table a deployment would ship ([[tab:beta_heldout]]): β bisected once per quality index on "
        ...
        r"β came from.")
    par(r"The held-out β misses the budget on the low side, at every rate: "
        r"it delivers between \HeldDbBest and \HeldDbWorst dB where 0.1 dB "
        r"was asked for, \HeldNUnder of the \HeldNHeld rates it covers "
        r"undershoot and \HeldNOver overshoot. That is the safe direction. A "
        r"decoder holding this table never spends quality the budget did not "
        r"allow; it spends less than it was allowed to, and buys less compute "
        r"with it. Distortion and saving move together, so those rows report "
        r"less saving, from \HeldGiveUpMin points at q\HeldGiveUpMinQp to "
        r"\HeldGiveUpMax at q\HeldGiveUpMaxQp. The last column shows that "
        r"nothing else is lost: re-bisecting on the test set to the quality "
        r"the held-out β actually delivered, the two allocations agree to "
        r"within \HeldTransferAbsMax points. What moves between the two sets "
        r"is the decibel a given β delivers, not where the allocation lands "
        r"once that decibel is fixed.")
    par(r"The highest rate is where this used to fail outright, and it is "
        ...
        r"\HeldFloorCalUnderMax dB, and the higher at the other "
        r"\HeldFloorNAbove, by \HeldFloorCalOverMax dB at "
        r"q\HeldFloorCalOverMaxQp. On an "
        r"earlier checkpoint that calibration floor sat above the budget, no "
        r"allocation on 512px photographs met 0.1 dB there, and the shipped "
        r"table had a hole where the highest rate should be. Four epochs of "
        r"training pulled the floor below the budget and the hole closed: "
        r"\HeldNNoBeta of the \HeldNRates rates are missing from the table "
        r"now. That difference is a quarter of the budget at "
        r"q\HeldFloorCalOverMaxQp and it is why the transfer undershoots "
        r"there. At the bottom of the ladder the two floors agree to within "
        r"\HeldFloorCalUnderMax dB, so the shortfall there is not the floor "
        r"but what a given β delivers above it.")
    ...
    par(r"The stem alone reaches \InputStemAgree, \InputStemOver over the "
        r"floor, and adding the latent, the scales and the bit count on top "
        r"of it reaches \InputAllAgree, which is not an improvement "
        r"([[tab:router_inputs]]). The signals are not independent: the "
        r"entropy model's scales and the bits spent are both functions of "
        r"the same latent the stem was computed from, so the head is being "
        r"given one piece of information in four dressings. That is worth "
        r"stating beside Section 5.6, where a rule reading the bit count "
        r"alone beats the whole head: the bit count is not a weak input, it "
        r"is the same input, read by something that does not have to learn "
    ...
    par(r"It replicates on a second training run. BEST is a separate recipe "
        r"at a different epoch, with its own router trained the same way. "
        r"There the calibrated bit rule beats the trained head at "
        ...
        r"everywhere ([[tab:raterank_best]]). The margins there are wider "
        r"than here, and the one rate it concedes is the only rate on either "
        r"checkpoint where the head finishes ahead, by under a point. We do "
        r"not read that as showing a learned head cannot be worth its "
        r"parameters, only that neither of our training runs produced one "
        r"that is. The calibrated bit rule stays close to the oracle on "
        r"both.")
    tbl("hybrid",
    ...
    par(r"<b>Whether the two signals are complementary.</b> They are not. We "
        ...
        r"for itself anywhere, and handing the decision to the head costs "
        r"between \BlendCostMin and \BlendCostMax points, the most at "
        r"\BlendCostMaxQ. The reason is that they are not two signals. The "
        r"head reads the entropy model's scales, and the bit count is what "
        r"those scales produce, so the head is a learned re-derivation of the "
        r"quantity it is being blended with. Mixing a noisier copy of a signal "
        r"into the signal cannot add information, and the table shows it "
        r"subtracting some.")
    par(r"<b>How much a trained head varies, and what that does to the "
        ...
        r"arithmetic model, because only one of them was hook-counted. At "
        r"\HeadSpreadWiderN of the \HeadSpreadRatesN rates that spread is "
        r"wider than the rule's margin over whichever head is nearer. The "
        r"claim is not that no learned router can beat the bit rule, but that "
    ...
    par(r"Most of the gap is cheap. Overriding a tenth of the tiles for "
        ...
        r"useful ones.")
    par(r"A partial map does not beat a complete one here, and the reason "
        r"it could is worth stating anyway. Signalling half the tiles for "
        r"\HybridBitsHalf bits lands \HybridHalfShortLo points short of "
        r"signalling all of them at q\HybridHalfShortLoQp and "
        r"\HybridHalfShortHi short at q\HybridHalfShortHiQp, at the same "
        r"distortion: it beats the full map at \HybridHalfVsFullN of "
        r"\HybridHalfVsFullOf rates. Nothing forces that ordering, because the "
        r"hybrid has <i>two</i> multipliers where A has one: the oracle's λ "
        r"governs the overridden tiles and the router's fixed β the rest. A "
        r"two-multiplier allocation reaches points on the distortion–compute "
        r"plane that a single Lagrangian sweep cannot, for the same reason "
        r"the sweep misses interior Pareto points at all, so a partial map "
        r"beating a complete one is not a contradiction and we have measured "
        r"it happening on a decoder whose predictor was worse. Configuration "
        r"A is optimal among allocations reachable by one multiplier, and "
        r"that is a smaller set than the word suggests. What closes the gap "
        r"here is the predictor: with a head fitted to these weights B starts "
        r"\HybridStartGapLo–\HybridStartGapHi points below A, and there is "
        r"little left for a second multiplier to exploit.")
    ...
    par(r"A better predictor concentrates its regret, which is what the "
        ...
        r"Both sides are measured on the reported weights and differ only "
        r"in which head C is built on, which is what the claim is about. "
    ...
    par(r"<b>Across rate.</b> Here the map does have to be recomputed, and the "
        ...
        r"wasteful: the q63 map applied at q0 delivers \TransferBackDb dB "
        r"and only \TransferBackSaving% where \ClassLowUvg% was available. Shallow exits are cheap in quality at low "
    ...
    tbl("latency",
        ...
        r"per-tile loop.")
    par(r"A saving in multiply-accumulates is not a saving in time ([[tab:complexity]]). Tiling "
        ...
        r"decode realises \WallSorted% at q0 where its operations predict "
        r"\WallPredicted%, and \WallHighMeasured% at q63 against "
        r"\WallHighPredicted%: about four fifths of the predicted saving "
        r"arrives at either end. What does not arrive is the tiling overhead "
        r"together with the bookkeeping of a shrinking active set, and a MAC "
        r"count sees neither. The shortfall scales with the saving rather "
        r"than sitting at a fixed offset, \WallShortLow points at the low "
        r"rate against \WallShortHigh at the high one. Both figures are the "
        r"sorted loop; written as the obvious one the low-rate decode "
        r"realises \WallMasked%, which is where the "
        r"\WallSortedGain points below come from.")
    figure("power.png",
    ...
    par(r"Energy follows time, not arithmetic ([[fig:power]]). At q0, q32 and q63 the routed "
    ...
    par(r"Sorting the tiles once by exit depth recovers part of it. The "
        ...
        r"mask, gathers the survivors and scatters the finished at every "
        r"group boundary, and each of those boundaries forces a "
        r"device-to-host synchronisation. Order the tiles by descending exit "
        r"depth and ``still active at group g'' becomes a contiguous prefix: "
        r"each group is then a slice instead of a gather, and one cumulative "
        r"count supplies every boundary, so no per-group mask is built at "
        r"all. The finished tiles come back through the inverse permutation "
        r"in a single scatter. The arithmetic is unchanged and the output is "
        r"bit-identical on GPU. At q0 the saving goes from \WallMasked% to "
        r"\WallSorted%, a gain of \WallSortedGain points, or a fifth of the "
        r"shortfall, for a change that touches no arithmetic "
        r"([[tab:latency]]).")
    ...
    par(r"Section 5.4 argued that once a budget saturates a ladder, the only "
        r"way to spend more is an exit that does not exist (, [[tab:runs]]). The table measures "
    ...
    par(r"So the ladder is a choice made at the operating point, not a "
        ...
        r"you which side of the crossover a given budget sits on. The two dotted lines in  are the two ceilings, and the crossing between them is the whole argument: a finer ladder is worse until the budget is large enough to reach past the coarse one's ceiling.")
    ...
    par(r"The difference panel is the part worth reading ([[fig:qualitative]]). Amplified twenty "
    ...
    par(r"The released decoder spends \RelGmac GMAC on a padded 1080p frame, "
        r"which is \RelKMacPx thousand multiply-accumulates for every pixel "
        r"it produces, and it spends the same on every pixel whatever the "
        r"picture is doing there ([[tab:released]]). At a 0.1 dB budget the "
        r"ladder takes that to \GmacAtOne GMAC, \KmacAtOne kMAC per pixel, "
        r"and delivers \DbAtOne dB below the release for a BD-Rate cost of "
        r"\BdRateALow%.")
    ...
    par(r"<b>The seam is reduced, and the exact remedy is not usable as it "
        ...
        r"because routing puts neighbouring tiles at different depths. "
    ...
    par(r"<b>The run had not converged when we measured it.</b> Every number "
        ...
        r"and is the wrong thing to watch: it is flat from the first epoch, "
        r"because the objective is dominated by a deepest exit with nothing "
        r"left to gain while what improves is the agreement between exits. "
        r"The supplement plots the two together.")
