...
def window():
    ...
    ax[1].legend(frameon=False, fontsize=5.0, handlelength=1.4, borderpad=0,
                 loc="upper left")
...
def concentration():
    ...
    for i, g in enumerate(gin):
        ax[1].text(i, g + 0.012, f"{g:.2f}", ha="center", fontsize=4.8,
                   color=ns.INK2)
    ax[1].set_xlabel("quality index")
...
def deciders():
    ...
    for i, q in enumerate(qs):
        ax[0].text(i, max(R[q], B[q]) + 0.7, f"+{R[q] - B[q]:.1f}",
                   ha="center", fontsize=4.6, color=ns.ORANGE)
    ax[0].set_xticks(x); ax[0].set_xticklabels([f"q{q}" for q in qs])
    ax[0].set_ylabel("decoder MACs saved (%)")
    ax[0].set_ylim(0, 34)
    ax[0].legend(frameon=False, fontsize=4.8, handlelength=1.1, borderpad=0,
                 loc="upper right")
    ns.panel(ax[0], "a")
    ...
    a1b.plot(xx, [100 * R[q] / A[q] for q in ok], "o-", ms=2.6, lw=0.9,
             color=ns.ORANGE)
    a1b.set_ylabel("% of the encoder search's saving", fontsize=5.8,
                   color=ns.ORANGE)
    a1b.tick_params(labelsize=5.4, length=2, width=0.5, colors=ns.ORANGE)
    a1b.set_ylim(0, 100)
    for sp in ("top",):
