...
def main(argv):
    ...
    ax[0].set_title(f"qp {a.qp}", fontsize=ns.fs(6), color=ns.INK2, loc="left")

    im = ax[1].imshow(km, cmap=cmap, norm=norm, interpolation="nearest")
    for (r, c), v in np.ndenumerate(km):
        ax[1].text(c, r, str(v), ha="center", va="center", fontsize=ns.fs(6),
                   color="white" if v >= K - 2 else ns.INK)
    ax[1].set_title(f"{saved:.1f}% saved, {k_sel_db:.3f} dB", fontsize=ns.fs(6),
                    color=ns.INK2, loc="left")

    im2 = ax[2].imshow(pen, cmap="viridis", interpolation="nearest")
    ax[2].set_title("dB per tile", fontsize=ns.fs(6), color=ns.INK2, loc="left")
    cb = fig.colorbar(im2, ax=ax[2], fraction=0.046, pad=0.02)
    cb.ax.tick_params(labelsize=ns.fs(6))
    ...
    ax[0].legend(handles=[Patch(facecolor=ramp[i],
                 ...
                 loc="upper center", bbox_to_anchor=(0.5, -0.02), fontsize=ns.fs(6),
