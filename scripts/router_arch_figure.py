...
def box(x, y, w, h, kind, title, sub=None, note=None, fs=5.6):
    ...
    if sub:
        ax.text(x + w / 2, y + h - 4.4, sub, ha="center", va="top", fontsize=6,
                color=ns.INK2, zorder=3)
    if note:
        ax.text(x + w / 2, y - 1.5, note, ha="center", va="top", fontsize=6,
                color=ns.INK2, zorder=3)
    return x + w, y + h / 2
...
ax.text(4.0, 38.6, "what the decoder already has", fontsize=6, color=ns.INK,
        va="top")
...
ax.text(26.9, 38.6, "projected, then pooled to one vector per tile",
        fontsize=6, color=ns.INK, va="top")
...
ax.text(65.25, 20.9, "concatenate", ha="center", va="center", fontsize=6,
        color=ns.INK, rotation=90)
ax.text(65.25, 4.9, f"{2*r_stem} + {2*r_lat} + 1 = {d_in}", ha="center",
        va="top", fontsize=6, color=ns.INK2)
...
for title, sub in mlp_rows:
    ...
    ax.text(80.75, y - 1.45, title, ha="center", va="center", fontsize=6,
            color=ns.INK, zorder=3)
    ax.text(80.75, y - 3.1, sub, ha="center", va="center", fontsize=6,
            color=ns.INK2, zorder=3)
    if y < 35.0:
...
ax.text(80.75, 10.55, "+ load-balancing bias", ha="center", va="center",
        fontsize=6, color=ns.INK, zorder=3)
ax.text(80.75, 8.9, "6 numbers, nudged by usage not by gradient",
        ha="center", va="center", fontsize=6, color=ns.INK2, zorder=3)
...
ax.add_patch(FancyBboxPatch((91.4, 6.6), 8.0, 28.6,
                            ...
                            fc=FACE["out"], ec=EDGE["out"], lw=0.7, zorder=2))
ax.text(95.4, 30.2, "logits  z", ha="center", va="center", fontsize=6,
        color=ns.INK, zorder=3)
ax.text(95.4, 27.6, f"one per exit, K={K}", ha="center", va="center",
        fontsize=6, color=ns.INK2, zorder=3)
ax.text(95.4, 21.5, "exits below j\nmasked to −∞", ha="center", va="center",
        fontsize=6, color=ns.INK2, zorder=3)
ax.text(95.4, 14.0, "argmax", ha="center", va="center", fontsize=6,
        color=ns.INK, zorder=3)
...
ax.text(50, 1.0, f"{n_par:,} parameters in total, and 0.162% of the decode it "
                 f"is deciding about", ha="center", va="bottom", fontsize=6,
        color=ns.INK2)
