...
def main(argv):
    ...
    a0.set_title(f"frame, {W0}×{H0}, padded to {W}×{H}", fontsize=ns.fs(6),
    ...
    a1.set_title(f"stem feature after {j} shared blocks, {fw}×{fh}×{f.shape[1]}",
                 fontsize=ns.fs(6), color=ns.INK2, loc="left", pad=3)
    ns.panel(a1, "b")
    ...
    for n, t in enumerate(pick):
        ...
        ax.set_xlabel(f"tile {t}", fontsize=ns.fs(6), color=ns.INK2, labelpad=1)
        if n == 0:
    ...
    def box(cx, w, txt, sub, colour, y=14.5, h=8.2):
        ...
        ax.text(cx, y + 1.5, txt, ha="center", va="center", fontsize=ns.fs(6),
                color=ns.INK, zorder=3)
        ax.text(cx, y - 2.0, sub, ha="center", va="center", fontsize=ns.fs(6),
    ...
    ax.add_patch(FancyArrowPatch((90, 8.4), (11, 8.4), arrowstyle="-|>",
                                 ...
                                 shrinkA=2, shrinkB=2, zorder=1))
    # Wrapping the sentence below made it four lines tall, and at y=1.0 it
    # reached this line. Both moved apart rather than either shrinking.
    ax.text(50, 6.6, f"unpatchify: view [1, {nh}, {nw}, {C_}, {Fp}, {Fp}] "
                     f"→ permute → reshape [1, {C_}, {fh}, {fw}]",
            ha="center", va="center", fontsize=ns.fs(6), color=ns.GREEN)
    # The sentence that stood here was 230 characters of prose drawn inside
    # the panel. Unwrapped it made savefig(bbox_inches="tight") export a
    # canvas 10.1 inches wide for a figure that asked for 7.2, so the picture
    # printed at two thirds of its drawn size; wrapped, four lines of 6 pt in
    # a panel 54 points tall ran into the line above. It is prose either way,
    # and prose belongs in the caption, which is where it now is.
