...
def content(k):
    ...
    # -------------------------------------------------------------- D.1
    k.h2("Conventions, the test set and the ladder")

    k.par(
        "Four terms are used throughout and are worth fixing before any "
        "number is read.")
    k.bullets([
        "<b>Saving.</b> The multiply-accumulate cost of the routed decode as "
        "a percentage below the released DCVC-UF intra decoder [14]. The "
        "released decoder is 1.0 and our own deepest exit is 1.0095 "
        "(results/router_RECIPE512_b01.json, pinned), because "
        "the adapters and the seam-repair module run on top of it, so a "
        "saving of zero against our deepest exit is a saving of minus 0.95% "
        "against the release. Every saving in this section is against the "
        "release, which is the smaller of the two and the one a reader can "
        "compare with a published decoder.",
        "<b>Budget.</b> The quality the allocation is allowed to give up, in "
        "dB of PSNR below the released decoder on the same bitstream. One "
        "cost multiplier λ is bisected until the whole test "
        "set lands on the budget; it is not re-bisected per class or per "
        "sequence, so a class or a sequence can be well above or well below "
        "the budget at the operating point that meets it on average.",
        "<b>Averaging convention.</b> A decibel can be averaged in two ways, "
        "and they disagree. The per-frame convention averages a per-frame "
        "decibel, which is what the released decoder's own evaluation script "
        "does and therefore what a published DCVC-UF number means. The "
        "pooled convention forms one distortion over every tile of every "
        "frame and takes the decibel of that, which is the form the "
        "Lagrangian is written in. Pooling always reports the smaller number, "
        "so it always flatters. Unless a table says otherwise, every dB here "
        "is the per-frame convention.",
        "<b>Exit.</b> The trunk is 12 blocks and carries six exits at "
        "two-block intervals. Split depth j = 2 means the first two exits sit "
        "before the point at which decoding becomes per tile, so they cannot "
        "be selected; four exits are selectable, e2 to e5, skipping 6, 4, 2 "
        "and 0 blocks. That is why the histograms below have four columns "
        "and not six.",
    ])

    t = k.rows(_testset_rows(k),
               "The test set, as the decoder sees it. Tiles are 256 px, and "
               "a frame is padded on its right and bottom edges until the "
               "feature map divides by the tile, so a 416x240 frame is "
               "decoded at 512x256 and splits into two tiles. The tile count "
               "is what limits how much an allocation can do: with two tiles "
               "and four selectable exits a whole frame has sixteen possible "
               "exit maps, against four to the fortieth power at 1080p. One "
               "pass over the set is 1765 tiles.")
    k.note("Classes, resolutions, tile counts and sequence counts from "
           "results/supp_per_class_budgets.json; padded sizes from "
           "results/tile_definition.json. Both on " + PINNED + ". Sequences "
           "are UVG [25], MCL-JCV [26] and HEVC classes B, C, D and E [9], "
           "one intra frame each.")

    t = k.rows(_ladder_rows(k),
               "The exit ladder. Blocks are trunk blocks of the released "
               "decoder, of which there are 12. Exits e0 and e1 exist in the "
               "network but are before the split and cannot be selected at "
               "this split depth. The adapter is the module that re-conditions "
               "the truncated feature map for the reconstruction head; it is "
               "the FFN variant where four or more blocks are skipped and a "
               "1x1 residual otherwise, and there is none at the deepest exit "
               "because nothing is skipped there.")
    k.note("results/adapter_cost.json, on " + PINNED + ", read by forward "
           "hooks on the modules themselves rather than from a written-down "
           "architecture.")
    ...
    # -------------------------------------------------------------- D.6
    k.h2("The frontier at every budget")

    k.par(
        "Sweeping the price of compute traces the achievable set. The "
        "construction is Shoham and Gersho's [28], later standard in image "
        "and video coding [29]: minimising D + λC per tile at "
        "a fixed price and sweeping the price recovers the lower convex hull "
        "of the operational set, and a budget that falls in a gap of the "
        "hull is met by the nearest reachable point rather than exactly. "
        "Reporting the whole sweep rather than one point on it is what the "
        "rate-distortion-complexity literature asks for [35, 36].")

    t = k.rows(_grid_rows(k),
               "The frontier on the pinned checkpoint: compute saved against "
               "the released decoder at nine budgets and five rates. A dash "
               "is a budget that cannot be met at that rate, because the "
               "deepest exit already gives up more quality than the budget "
               "allows. The three lowest rates are unreachable at 0.05 dB "
               "for that reason. Reading down a column shows the return on "
               "loosening the budget flattening out as the ceiling is "
               "approached; reading across a row shows the cost of moving "
               "from the lowest rate to the highest at a fixed budget, "
               "which is 14.2 points at 0.1 dB and between 12 and 17 "
               "points elsewhere.")
    k.note("results/signalled_RECIPE512_grid.json, on " + PINNED + ", 53 "
           "sequences at two frames each. Decibels are the per-frame "
           "convention. This is the densest budget sweep in this work that "
           "is on the pinned checkpoint, and it is the source for the 0.1 dB "
           "and 0.3 dB columns the main paper reports.")

    t = k.rows(_operating_rows(k),
               "The window the budget has to live in, at all nine rates "
               "measured rather than the five the main paper prints. The "
               "floor is the quality lost with every tile at the deepest "
               "exit, which is not zero because the ladder is trained and "
               "the deepest exit drifts. Saturation is the point at which "
               "every tile has reached the shallowest exit and no further "
               "saving exists. The last column places the 0.1 dB budget "
               "inside that window: it is 45% of the way up the band at q0 "
               "and 9% at q63, which is the same fact as the falling saving "
               "seen from the other side.")
    k.note("results/saturation_RECIPE512_ctc53.json, on " + PINNED + ", 53 "
           "frames.")

    t = k.rows(_curve_rows(k),
               "The budget sweep on the epoch-1 checkpoint, which measures "
               "seven budgets against the grid's nine but carries "
               "per-sequence detail the grid does not. A star marks a cell "
               "where the budget is above saturation, so the value is the "
               "ceiling rather than a response to the budget, and a cell "
               "marked n/a is a budget below the floor at that rate. This "
               "file was measured under an earlier compute-cost model and saturates "
               "at 42.5%, where the pinned files saturate at \\Ceiling%, so "
               "the two tables should be compared by shape and not by level.")
    k.note("results/curve_RECIPE512.json, on runs/RECIPE512/ckpt_eval.pth.tar "
           "at epoch 1, 53 sequences at one frame each. Savings are against "
           "our own deepest exit in this file rather than against the "
           "release, a difference of about 0.6 points at the ceiling. At the "
           "0.1 dB operating point the pooled convention reads 0.100 dB "
           "where the per-frame convention reads 0.119 dB at q0 and 0.123 dB "
           "at q63, which is the convention difference the first subsection "
           "warns about, measured on this file.")
