...
a.text(1, 0.108, "the 0.1 dB budget this project works to", fontsize=5,
       color=ns.INK)
...
a.text(31, 0.545, "nothing left to buy — the budget is being thrown away",
       fontsize=5.2, ha="center", color="#666666")
a.text(31, (flo[4] + sat[4]) / 2, "the band where a budget buys compute",
       fontsize=5.2, ha="center", color=ns.GREEN)
a.text(31, 0.018, "infeasible: tiling alone already costs this much",
       fontsize=5.2, ha="center", color=ns.VERM)
a.set_ylim(0, 0.62); a.set_xlim(qp[0], qp[-1])
...
b.set_title("The band WIDENS with rate — the ladder has more to offer at\n"
            "high rate, and the 0.1 dB budget uses less and less of it",
            fontsize=6, color=ns.INK2, loc="left")
used = (0.1 - flo) / (sat - flo)
b2 = b.twinx()
b2.plot(qp, 100 * used, marker="o", color=ns.BLUE, lw=1.0, ls=(0, (3, 2)))
b2.set_ylabel("share of the band a 0.1 dB budget uses (%)", color=ns.BLUE)
...
b.text(qp[0] + 1, (sat - flo).max() * 0.10,
       f"ceiling is {CEIL:.2f}% everywhere in the grey region",
