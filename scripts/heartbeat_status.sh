...
while true; do
  ...
  MX=$(echo "$OUT" | grep -oE 'check_mixed: [A-Za-z0-9 ()]+' | head -1 | sed 's/check_mixed: //')
  PV=$(echo "$OUT" | grep -oE 'check_provenance: [A-Za-z0-9 ()]+' | head -1 | sed 's/check_provenance: //')
...
" 2>/dev/null)
  ...
  echo "$TS  fix $FIX | claims $CHK twins $TW tex $TX prose $PR layout $LY figs $FR render $RN figprose $FP overlap "$FO" cites $CT numbers $NU figclaims $FC mixed $MX prov "$PV" | paper ${MAIN}p supp ${SUPP}p | gpu ours=$OURS others=$OTHER | newest ckpt $AGE | ep $EP | scratch ${SC:--} pids=$SCALIVE"
  sleep "$INTERVAL"
done
