...
" 2>/dev/null)
  ...
  echo "$TS  fix $FIX | claims $CHK twins $TW tex $TX prose $PR layout $LY figs $FR render $RN figprose $FP overlap "$FO" cites $CT numbers $NU figclaims $FC mixed $MX | paper ${MAIN}p supp ${SUPP}p | gpu ours=$OURS others=$OTHER | newest ckpt $AGE | ep $EP | scratch ${SC:--} pids=$SCALIVE"
  sleep "$INTERVAL"
done
