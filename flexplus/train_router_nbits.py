"""The same trainer, with the neighbourhood-aware head.

A copy rather than a flag: nothing under ~/FLEX-UF changes for an experiment
that is not in the paper. The only differences from
scripts/train_router2.py are the head class, the extra --tile_lat it needs,
and that the paths default under this branch. See neighbour_bits.py for what
the head adds and why.
"""

import argparse, json, math, os, sys, time
from pathlib import Path
import torch
# The decoder, the oracle and the data loader all live in the main tree and
# are imported read-only; this branch adds a head and nothing else.
...
from flexuf.router.head2 import oracle_ce_loss
sys.path.insert(0, str(Path.home() / 'FLEX-PLUS'))
from flexplus.neighbour_bits import NeighbourHead
...
ap.add_argument("--label", default="nbits",
...
head = NeighbourHead(384, 256, cfg.num_exits, min_exit=cfg.split_depth,
                     inputs=a.inputs, tile_lat=cfg.latent_patch).to(dev)
