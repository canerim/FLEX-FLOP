...
# A COPY of ~/FLEX-UF/train_flexuf_image.py, living in FLEX-PLUS so the branch
# experiment cannot touch the main one. Only the exit distribution changes; the
# recipe, the data and the schedule are the original's, verbatim.
sys.path.insert(0, str(Path.home() / "FLEX-UF"))
...
# --------------------------------------------------------------------------
# The only change: train the ladder for the distribution it is deployed at.
#
# At the 0.1 dB operating point the exits are used 0 / 0 / 71 / 21 / 4.3 / 3.6
# per cent, while `exit_weights` gives all six the same alpha and
# `forward_random_depth` samples U{j..K-1}. KL(deployed || training) is 0.966
# nat on the auxiliary path and 0.559 on the RD path. CalexNet
# (arXiv:2509.08318) identifies exactly this covariate shift for early-exit
# branches on a frozen backbone and corrects it by importance weighting onto
# the distribution that actually reaches each branch; IMTA (ICCV 2019) shows
# the same weighting is what balances conflicting per-exit gradients.
#
# Both paths are corrected here, and nothing else is.
_PRIOR = None
_orig_exit_weights = exit_weights
_orig_frd = FlexUFIntra.forward_random_depth


def exit_weights(num_exits, aux_weight, schedule, epoch, warmup_epochs, device):
    w = _orig_exit_weights(num_exits, aux_weight, schedule, epoch,
                           warmup_epochs, device=device)
    if _PRIOR is None:
        return w
    p = _PRIOR.to(device).clone()
    # Keep the deepest exit's weight where the original put it (1.0) and
    # redistribute the auxiliary mass by usage, so total auxiliary weight --
    # and therefore the balance against the anchor and distillation terms --
    # is unchanged. Only its allocation across rungs moves.
    aux_mass = float(w[: num_exits - 1].sum())
    q = p[: num_exits - 1]
    q = q / q.sum() * aux_mass if float(q.sum()) > 0 else w[: num_exits - 1]
    w[: num_exits - 1] = q
    return w


def _frd(self, x, qp):
    """forward_random_depth with the depth drawn from the deployed prior."""
    if _PRIOR is None:
        return _orig_frd(self, x, qp)
    K, j = self.cfg.num_exits, self.cfg.split_depth
    real = torch.randint

    def fake(low, high, size, **kw):
        if int(low) == j and int(high) == K:
            pr = _PRIOR[j:K].to(x.device).clamp_min(1e-8)
            return torch.multinomial(pr / pr.sum(), int(size[0]),
                                     replacement=True).add_(j)
        return real(low, high, size, **kw)
...
FlexUFIntra.forward_random_depth = _frd
...
def parse_args(argv):
    ...
    p.add_argument("--exit_prior", type=str, default="",
                   help="comma-separated usage over the K exits at the "
                        ...
                        "weighting, which is the control arm")
...
def main(argv):
    ...
    global _PRIOR
    if args.exit_prior:
        _PRIOR = torch.tensor([float(v) for v in args.exit_prior.split(",")],
                              dtype=torch.float32)
        _PRIOR = _PRIOR / _PRIOR.sum()
        print("exit prior: " + " ".join(f"e{i}:{float(v):.4f}"
                                        for i, v in enumerate(_PRIOR)),
              flush=True)
    cfg = build_cfg(args)
